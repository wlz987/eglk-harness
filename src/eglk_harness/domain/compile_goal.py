...
def compile_goal(
    ...
    *,
    ...
    if backend not in {"mock", "fake"}:
        ...
        if use_llm:
            try:
                llm_text = asyncio.run(_compile_via_adapter(workdir, goal_text, backend))
            except RuntimeError:
                # nested event loop — skip LLM enrich
                llm_text = None
            except Exception:
                llm_text = None
        ...
        if use_llm:
            try:
                llm_text = asyncio.run(_compile_via_adapter(workdir, goal_text, "mock"))
            except Exception:
                llm_text = None
