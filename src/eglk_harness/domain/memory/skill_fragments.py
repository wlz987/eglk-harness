...
class DisclosureLevel(str, Enum):
    """How much of a role skill template to inject."""
...
def _fragment_ids_for_workdir(workdir: Path | None) -> list[str]:
    ...
    if workdir is not None:
        ...
        if (mcp_dir / "wa-browser.mcp.json").is_file():
            if "wa-browser" not in ids:
    ...
    # Map suite names to default fragments when not already fragment ids
...
def _load_fragment_text(fragment_id: str) -> str | None:
    ...
    if not safe:
        return None
    here = Path(__file__).resolve().parent / "skill_fragments" / f"{safe}.md"
    if here.is_file():
        return here.read_text(encoding="utf-8").strip()
    try:
        root = resources.files("eglk_harness.domain.memory.skill_fragments")
