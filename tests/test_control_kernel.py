...
def test_tokens_from_codex_jsonl_prefers_total() -> None:
    ...
    assert tokens_from_codex_jsonl(stream) == 20
...
def test_format_goal_frame_truncates_long_source_with_marker() -> None:
    goal = "# Long goal\n\n## Constraints\n- " + "x" * 9000
    text = format_goal_frame(goal)
    assert "…[truncated]…" in text
    assert "WA_BROWSER_TASK_ID" not in text
...
def test_repair_counts_and_focus(tmp_path: Path) -> None:
    ...
    assert repair_counts_from_decisions(tmp_path, subgoal_id="root")["__all__:incomplete"] == 2
...
def test_sigma_refined_flush_to_candidates(tmp_path: Path) -> None:
    from eglk_harness.domain.product.init_project import init_project
    from eglk_harness.domain.kernel import paths as kernel_paths

    init_project(tmp_path)
    loop = kernel_paths.loop_goal_dir(tmp_path, "g1")
    (loop / "sigma" / "refined").mkdir(parents=True)
    (loop / "sigma" / "refined" / "000.json").write_text(
        json.dumps({"id": "sigma-hit-000", "kind": "hit", "text": "lesson a"}),
        encoding="utf-8",
    )
    (loop / "sigma" / "refined" / "001.json").write_text(
        json.dumps({"id": "sigma-hit-001", "kind": "hit", "text": "lesson b"}),
        encoding="utf-8",
    )
    n = sigma.merge_refined_into_active(tmp_path, loop)
    assert n == 2
    cand_dir = kernel_paths.memory_lifecycle_dirs(tmp_path)["candidate"]
    written = list(cand_dir.glob("*.json"))
    assert len(written) >= 2
    assert not list((loop / "sigma" / "refined").glob("*.json"))
